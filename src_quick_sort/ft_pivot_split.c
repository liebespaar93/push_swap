...
/*   Updated: 2022/08/29 17:35:06 by kyoulee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	while (!(pivot->deep == 0 && pivot->a_hold))
	{
		if (a->head && pivot->a_hold != a->tail)
			ft_pivot_split_a_right(a, b, &pivot);
		else if (b->head && pivot->b_hold != b->tail)
			ft_pivot_split_b_right(a, b, &pivot);
		else if (a->head && pivot->tail == a->head)
			ft_pivot_split_a_left(a, b, &pivot);
		else if (b->head && pivot->tail == b->head)
...
int	ft_pivot_split_b_right(t_d_list_header *a, t_d_list_header *b, \
	t_pivot **pivot)
{
	if (b->tail->prev == (*pivot)->b_hold && ft_rrb(b) && ft_pa(a,b))
	{
		ft_ra(a);
		(*pivot)->a_hold = a->tail;
		return (0);
	}
	ft_pivot_add(pivot, ft_pivot_new(b->tail, b->tail));
	if (a->head == NULL)
		(*pivot)->a_hold = b->tail;
	ft_rrb(b);
	ft_pa(a,b);
	while ((*pivot)->b_hold != b->tail && ft_rrb(b) && ft_pa(a,b))
		if (*((*pivot)->head->data) >= *a->head->data)
			ft_ra(a);
	(*pivot)->tail = a->head;
	if (b->head == NULL)
		(*pivot)->b_hold = NULL;
	...
	t_pivot **pivot)
{
	...
	if (temp.head == temp.tail && ft_ra(a))
	{
		(*pivot)->a_hold = a->tail;
		return (-1);
	}
	...
	if ((*pivot)->b_hold == NULL)
		(*pivot)->b_hold = a->head;
	ft_pb(a,b);
	while (temp.head != b->tail && ft_pb(a,b))
	{
		if (*(*pivot)->head->data >= *b->head->data)
			ft_rb(b);
	}
	...
	t_pivot **pivot)
{
	...
	temp = **pivot;
	(*pivot) = ft_pivot_del(pivot);
	if (temp.head == temp.tail && ft_pa(a,b) && ft_ra(a))
	{
		(*pivot)->a_hold = a->tail;
		return (-1);
	}
	...
	if ((*pivot)->a_hold == NULL)
		(*pivot)->a_hold = b->head;
	ft_pa(a,b);
	while (temp.head != a->tail && ft_pa(a,b))
	{
		if (*(*pivot)->head->data >= *a->head->data)
			ft_ra(a);
	}
